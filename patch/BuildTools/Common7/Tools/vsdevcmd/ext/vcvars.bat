...
@REM Add MSVC
set "__VCVARS_DEFAULT_CONFIG_FILE_NAME=Microsoft.VCToolsVersion.default.txt"
set "__VCVARS_DEFAULT_CONFIG_FILE=%VCINSTALLDIR%Auxiliary\Build\%__VCVARS_DEFAULT_CONFIG_FILE_NAME%"
...
@REM 自定义配置优先级最高
if defined __VCVARS_CUSTOM_CONFIG_FILE_NAME (
    if EXIST "%VCINSTALLDIR%Auxiliary\Build\%__VCVARS_CUSTOM_CONFIG_FILE_NAME%" (
        if "%VSCMD_DEBUG%" GEQ "2" @echo [DEBUG:ext\%~nx0] %__VCVARS_CUSTOM_CONFIG_FILE_NAME% was found.
        set "__VCVARS_DEFAULT_CONFIG_FILE=%VCINSTALLDIR%Auxiliary\Build\%__VCVARS_CUSTOM_CONFIG_FILE_NAME%"
    )
) else (
    if "%VSCMD_DEBUG%" GEQ "1" @echo [DEBUG:ext\%~nx0] %__VCVARS_CUSTOM_CONFIG_FILE_NAME% was not found. Defaulting to 'Microsoft.VCToolsVersion.default.txt'.
)
